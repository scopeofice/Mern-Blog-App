...
const CommentItem = ({ comment, activeUser }) => {
    ...
    useEffect(() => {
        ...
        const getCommentLikeStatus = async () => {
            ...
            try {
                const { data } = await axios.post(`http://13.232.167.112:5000/comment/${comment_id}/getCommentLikeStatus`, { activeUser }, {
                    headers: {
    ...
    const handleCommentLike = async () => {
        ...
        try {
            const { data } = await axios.post(`http://13.232.167.112:5000/comment/${comment_id}/like`, { activeUser }, {
                headers: {
                    "Content-Type": "application/json",
                    authorization: `Bearer ${localStorage.getItem("authToken")}`,
                },
    ...
    return (
        ...
        <div className='comment-item'>
            <div className="comment-top-block">
                ...
                <section>
                    <img src={`http://13.232.167.112:5000/userPhotos/${comment.author.photo}`} alt={comment.author.username} width="35" />
